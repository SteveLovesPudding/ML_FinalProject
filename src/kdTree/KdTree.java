...
public class KdTree<T extends KdTree.XYZPoint> {
	...
	private static KdNode createNode(List<Point> list, int k, int depth) {
		...
		if (list.size() > 0) {
			...
			List<Point> less = new ArrayList<Point>(list.size() - 1);
			List<Point> more = new ArrayList<Point>(list.size() - 1);
	 ...
	 */
	public boolean remove(Point value) {
		if (value == null)
			return false;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Collection<T> nearestNeighbourSearch(int K, Point value) {
		if (value == null)
			return null;
	...
	public static class KdNode implements Comparable<KdNode> {
		...
		public KdNode(int k, int depth, Point id) {
			this(id);
			this.k = k;
			this.depth = depth;
		}
